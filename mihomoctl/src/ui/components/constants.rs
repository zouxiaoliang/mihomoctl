...
mod tests {
    ...
    #[test]
    fn proxy_tree_indicators_use_terminal_safe_ascii() {
        for indicator in [
            Consts::FOCUSED_INDICATOR,
            Consts::FOCUSED_EXPANDED_INDICATOR,
            Consts::UNFOCUSED_INDICATOR,
            Consts::EXPANDED_FOCUSED_INDICATOR,
        ] {
            assert!(
                indicator.is_ascii(),
                "{indicator:?} may render as a missing-glyph box in terminals"
            );
        }
    }
}
