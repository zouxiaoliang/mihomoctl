...
- 首次使用无需先配置服务器：没有活跃服务器时进入 TUI 会自动打开服务器管理列表，按 `a` 即可添加
- 使用数字键在标签页之间切换
- 按空格键锁定列表，之后可以移动列表
- 在锁定模式下使用方向键移动列表
- 在 Proxies 页按 `t` 测试当前组的节点延迟，按 `Shift-T` 测试所有节点
- 按 `Shift-S` 打开服务器管理弹窗：方向键选择、回车切换，`a` 添加服务器，`d` 删除选中的服务器（需回车确认，当前活跃服务器不可删除）
- 按 `Shift-H` 打开操作指南，查看完整的快捷键说明（方向键滚动，`Shift-H` 或 Esc 关闭）
- 按 `Ctrl-D` 打开调试面板
