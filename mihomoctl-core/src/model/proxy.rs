...
#[test]
fn proxy_type_parses_current_mihomo_adapter_types() {
    let proxy_type = |value: &str| {
        serde_json::from_str::<ProxyType>(&format!("\"{}\"", value))
            .expect("proxy type should deserialize")
    };

    assert_eq!(proxy_type("Vless"), ProxyType::Vless);
    assert_eq!(proxy_type("Hysteria2"), ProxyType::Hysteria2);
    assert_eq!(proxy_type("WireGuard"), ProxyType::WireGuard);
    assert_eq!(proxy_type("Tuic"), ProxyType::Tuic);
    assert_eq!(proxy_type("AnyTLS"), ProxyType::AnyTLS);

    assert!(ProxyType::Vless.is_normal());
    assert!(ProxyType::AnyTLS.is_normal());
}
